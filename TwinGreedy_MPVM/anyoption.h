...
public: /* the public interface */
	 ...
	 * 以下set方法指定特殊字符和分隔符（如果未设置），将使用传统默认值
	 *
         */
	 ...
	 * provide the input for the options
	    ...
	    提供选项的输入，例如comvndline的argv []和要使用的选项文件名；
	 */
	void useCommandArgs( int _argc, char **_argv );
	//processCommandArgs函数实际调用的函数，参数是argv数组，通过读取存在argv数组中的命令行的来获取option

    void useFiileName( const char *_filename );
    //processFile函数实际调用的函数，参数是文件路径，通过读取存储option的文件来获取option
	...
	/* 
         ...
         *
         * 关闭POSIX样式选项，这意味着以'-'或“-”开头的任何内容都将被视为有效选项
         * 这也意味着您无法将一堆POIX选项字符加在一起，例如“ -l -r”的“ -lr” ”
         * 即-后面的所有选项看作一个整体
         *
         */
	 ...
	 * the options can be either a string ( GNU style )
         ...
         * or both ( --width, -w )
         *
         * option可以写到命令行或者写到optionfile中，或者都可以，这由你选择
	 ...
	 * 使用这几个函数set的option，允许从命令行和file中读取
	 * */
	...
	/*
         ...
         */
	void processCommandArgs( int _argc, char **_argv );//参数是argv数组，通过读取存在argv数组中的命令行的来获取option
	void processCommandArgs( int _argc, char **_argv, int max_args );
	bool processFile( const char *_filename );//参数是文件路径，通过读取存储option的文件来获取option
